...
public class Playmore {
	...
	private PlaymoreClient getController(GameDTO initialGameDTO, long accountID)
			throws IOException {
		...
		if (teamOne.size() + teamTwo.size() == 1) {// i am the host
			return new HostPlaymoreController(null, null, accountID);
		} else {
			return new PlayerPlaymoreController(null, null, accountID);
		}
	}
